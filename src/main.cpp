...
static void sendSensorValueForAllComponents() {
  ...
  lora.endMsg();
  Serial.println(F("\nSent sensor values for all components"));
}
...
static void sendConfigValues(const IComponent* component) {
  ...
  ValueItemT items[13];
  uint8_t numberOfConfigValues = component->getConfigValueItems(items, 13);

  if (numberOfConfigValues > 0) {
    lora.beginValueMsg();
    for (uint8_t i = 0; i < numberOfConfigValues; i++) {
      lora.addValueItem(items[i]);
    }
    lora.endMsg();
  }
}
...
void setup() {
  ...
  Serial.println(F("Welcome message:"));
  printWelcomeMsg();
  Serial.println();
